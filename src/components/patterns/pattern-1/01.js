...
const Clap01 = ({ animationTimeline }) => {
  const MAXIMUM_USER_CLAP = 50;
  const [clapState, setClapState] = useState(initialState);
  const { count, countTotal } = clapState;

  const handleClapClick = (e) => {
  ...
  return (
    <button className="clap" id="clap" onClick={handleClapClick}>
...
const Usage = () => {
  const AnimatedMediumClap = withClapAnimation(Clap01);
